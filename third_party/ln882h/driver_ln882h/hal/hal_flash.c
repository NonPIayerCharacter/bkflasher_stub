...
static uint8_t hal_flash_update_with_volatile(void)
{
    ...
    //1. read flash ID
    if(hal_flash_read_id() != TH25Q16HB_ID){
        return 0;
    }
...
uint32_t hal_flash_read_id(void)
{
    ...
    value = (read_back[0] << 16) | (read_back[1] << 8) | (read_back[2]); /* MID | MemType | Capacity */
    return value;
}
 ...
 * @param otp_area_num OTP area number, 
 *        TH25Q16UC, there are 3 OTP areas, so the otp_area_num should be 0,1 or 2.
 *        other flash, there is only one OTP area, so don`t care the otp_area_num.
...
void hal_flash_lock_otp(uint8_t otp_area_num)
{
    ...
    if (id == TH25Q16UC_ID || id == ZB25VQ16E_ID || id == TH25Q32HA_ID) {
        hal_assert(otp_area_num < 3);
        if (otp_area_num == 0) {
            status2.bits_3LB.LB1 = 1;
        } else if (otp_area_num == 1) {
            status2.bits_3LB.LB2 = 1;
        } else if (otp_area_num == 2) {
            status2.bits_3LB.LB3 = 1;
        }
    } else {
        status2.bits.LB = 1;
    }
 ...
 * @return if only one OTP ( 1:locked  0:unlock)
 *         TH25Q16UC has 3 OTP areas ( LB1<<1 | LB2<<2 | LB3<<3) will be returned
 */
uint8_t hal_flash_get_otp_state(void)
{
    ...
    if (id == TH25Q16UC_ID || id == ZB25VQ16E_ID || id == TH25Q32HA_ID) {
        ...
        status2.bits_3LB.LB2 = 1;
        status2.bits_3LB.LB3 = 1;
        lock_bit = ((status2.bits_3LB.LB1 << 0) | (status2.bits_3LB.LB2 << 1) | (status2.bits_3LB.LB3 << 2));
    } else {
        lock_bit = status2.bits.LB;
    }
...
void hal_flash_security_area_erase(uint32_t offset)
{
    ...
    uint32_t id = hal_flash_read_id();
    if(id == TH25Q16UC_ID || id == ZB25VQ16E_ID || id == TH25Q32HA_ID){
        hal_assert(offset < (1024*3));
        if (offset < 1024) {           //0~1023
            offset = offset | 0x1000;  //OTP area0 start from 0x1000
        } else if (offset < (1024*2)) {//1024~2047
            offset = offset | 0x2000;  //OTP area1 start from 0x2000
        } else if (offset < (1024*3)) {//2048~3071
            offset = offset | 0x3000;  //OTP area2 start from 0x3000
        }
    } else {
