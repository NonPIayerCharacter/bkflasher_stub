...
#if defined(__CONFIG_CPU_80MHZ) && (__CONFIG_CPU_80MHZ)
    clock_init.clk_pclk0_div    = CLK_PCLK0_2_DIV;  //80M /2=40M
    clock_init.clk_hclk_div     = CLK_HCLK_2_DIV;   //160M/2=80M
#else
    clock_init.clk_pclk0_div    = CLK_PCLK0_4_DIV;  //160M/4=40M
    clock_init.clk_hclk_div     = CLK_HCLK_NO_DIV;  //160M/1=160M
#endif
...
#if defined(__CONFIG_CPU_80MHZ) && (__CONFIG_CPU_80MHZ)
    hal_misc_cmp_set_ble_div_para_m1(4);  //keep BLEclk=16M (160M/10=16M ==> 80M/5=16M)
    hal_misc_cmp_set_ble_div_para_up(1);
#endif
}
